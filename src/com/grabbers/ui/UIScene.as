	...
	public class UIScene extends DisplayObjectContainer
	{
		...
		public function init():Boolean {
			...
			for (var i:uint = 0; i < list.length(); i++) {
				...
				var obj:UIObject = UIObjectFactory.createObject(modelName, scrW, scrH);				
				if (obj == null)
					continue;
				
				if (!obj.init(texPack, list[i], scrW, scrH))
					continue;
		...
		public function popupDialog(name:String):Boolean {
			...
			if (_objMap[name] != null) {
				...
				if (uiForm != null) {
					_layerMask.width = background!=null ? background.width : App.stage.stageWidth;
					_layerMask.height = background!=null ? background.height : App.stage.stageHeight;
		...
		public function execCommand(childName:String, command:String):void {
